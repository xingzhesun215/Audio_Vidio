package com.sun.eg8.video_encode_decode.encode.camera;
...
import com.sun.eg8.video_encode_decode.encode.AvcEncoder;
...
public class SimpleCameraPreview extends SurfaceView implements SurfaceHolder.Callback, Camera.PreviewCallback {
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        switch (mState) {
            ...
            case STATE_RECORD:
                ...
                if (mAvcEncoder == null) {
                    File outFile = getOutputMediaFile(MEDIA_TYPE_VIDEO);
                    filePath = outFile.getAbsolutePath();
                    DecodeH264ToTextureViewActivity.filePath = filePath;
                    DecodeH264ToSurfaceViewActivity.filePath = filePath;
                    mAvcEncoder = new AvcEncoder(mBestSize.height,
                            mBestSize.width, mFrameRate, outFile
                            , true);
    ...
    public File getOutputMediaFile(int mediaType) {
        ...
        if (mediaType == MEDIA_TYPE_IMAGE) {
            ...
            storageDir = new File("/sdcard/eg8/");
